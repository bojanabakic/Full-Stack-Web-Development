...
const App = () => {
  ...
  const addNewName = (event) => {
    ...
    if (persons.some(x => x.name === personObject.name)) {
      ...
      if (dialogBox) {
        let match = persons.filter(x => x.name === personObject.name)[0]

        personsServices
          .update(match.id, personObject)
          .then(() => {
            let newPeople = persons.filter(x => x !== match)
            newPeople = newPeople.concat(personObject)
          ...
          .catch((error) => {
            ...
            setMessage({ text: `${error.response.data.error}`, type: "failed" })
